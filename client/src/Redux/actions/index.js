...
export function getAllDogs() {
  return async function (dispatch) {
    var res = await axios.get(urlMyApi + "/dogs");
    return dispatch({
      type: "GET_ALL_DOGS",
      payload: res.data,
    });
...
export function getBreed(dogName) {
  return async function (dispatch) {
    try {
      var json = await axios.get(urlMyApi + "/dogs?name=" + dogName);
      return dispatch({
        type: "GET_BREED",
        payload: json.data,
      });
...
export function showDogDetails(id) {
  return async function (dispatch) {
    try {
      var json = await axios.get(urlMyApi + "/dogs/" + id);
      return dispatch({
...
export function postDog(payload) {
  return async function () {
    const data = await axios.post(urlMyApi + "/dog", payload);
    return data;
  };
}
